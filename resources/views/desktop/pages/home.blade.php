...
<div class="container m-auto flex justify-between items-start">
    <div class="w-4/12 flex items-start justify-center">
        <x-desktop.atoms.text-offer-card :index="'0'" icon="{{ Vite::asset('resources/images/booking.webp') }}"
            :description="__('Effortless Booking Anywhere in Morocco')" :title="__('Book Umrah from anywhere in Morocco')"></x-desktop.atoms.text-offer-card>
    </div>
    <div class="w-4/12 flex items-start justify-center">
        <x-desktop.atoms.text-offer-card :index="'1'" icon="{{ Vite::asset('resources/images/customer-offer.webp') }}"
            :description="__('Exclusive Packages and Seasonal Offers at Competitive Prices')" :title="__('Special packages and offers throughout the year')"></x-desktop.atoms.text-offer-card>
    </div>
    <div class="w-4/12 flex items-start justify-center">
        <x-desktop.atoms.text-offer-card :index="'2'" icon="{{ Vite::asset('resources/images/customer-service.webp') }}"
            :description="__('Our team is ready to answer all your inquiries and provide assistance at any time to ensure the comfort and tranquility of the pilgrims.')" :title="__('24/7 support service')"></x-desktop.atoms.text-offer-card>
    </div>
</div>
<x-desktop.atoms.spacer></x-desktop.atoms.spacer>
<div class="container m-auto flex justify-between items-start">
    <x-desktop.atoms.title :subtitle="__('Dedicated to Making Umrah and Hajj a Seamless Experience')"
        :title="__('welcome to our agency for the omrah in marrakech')"></x-desktop.atoms.title>
</div>
<x-desktop.atoms.spacer></x-desktop.atoms.spacer>
        ...
        <x-shared.carousel>
            @foreach ($hadjOffers as $hadjOffer)
                <div class="min-w-[350px] xl:w-3/12 lg:w-4/12 md:w-6/12 sm:w-6/12 xs:w-full p-3 pb-6">
                    <x-desktop.offer-card :data=$hadjOffer></x-desktop.offer-card>
                </div>
            @endforeach
        </x-shared.carousel>
    </div>
    ...
    <div class="container m-auto">
        <x-shared.carousel>
            @foreach ($umrahOffers as $umrahOffer)
                <div class="min-w-[350px] xl:w-3/12 lg:w-4/12 md:w-6/12 sm:w-6/12 xs:w-full p-3 pb-6">
                    <x-desktop.offer-card-with-preview :data=$umrahOffer></x-desktop.offer-card>
                </div>
            @endforeach
        </x-shared.carousel>
    </div>
