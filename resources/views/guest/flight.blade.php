    ...
    <div class="w-4/12 h-auto sticky top-10">
        <div class="border border-black-200 bg-white rounded-3xl p-6 mb-6 w-full">
            <span class="block opacity-45 text-base font-bold line-through line-clamp-2 lining-nums">{{ $flight->old_price }}
                {{__('index.dh')}}</span>
            <span class="text-3xl block font-black lining-nums">{{ $flight->price }} {{__('index.dh')}} {{__('index.For a person')}}</span>
            <span
            ...
            <div class="flex flex-col">
                ...
                <a href="{{route('faqs')}}" class="btn btn-lg text-xl rounded-xl border border-black-200 hover:shadow-lg font-black mb-3">{{__('index.You have a question?')}}</a>
            </div>
        </div>
...
<div class="container m-auto flex justify-between items-start">
    <div class="w-4/12 flex items-start justify-center">
        <x-text-offer-card icon="{{ Vite::asset('resources/images/booking.webp') }}" :description="__('index.Effortless Booking Anywhere in Morocco')" :title="__('index.Book Umrah from anywhere in Morocco')"></x-text-offer-card>
    </div>
    <div class="w-4/12 flex items-start justify-center">
        <x-text-offer-card icon="{{ Vite::asset('resources/images/customer-offer.webp') }}" :description="__('index.Exclusive Packages and Seasonal Offers at Competitive Prices')"
            :title="__('index.Special packages and offers throughout the year')"></x-text-offer-card>
    </div>
    <div class="w-4/12 flex items-start justify-center">
        <x-text-offer-card icon="{{ Vite::asset('resources/images/customer-service.webp') }}" :description="__('index.Our team is ready to answer all your inquiries and provide assistance at any time to ensure the comfort and tranquility of the pilgrims.')" :title="__('index.24/7 support service')"></x-text-offer-card>
    </div>
</div>
<x-spacer></x-spacer>
<x-footer></x-footer>
