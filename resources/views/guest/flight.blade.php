...
<x-page-header :title="__('Offer')" :breadcrumbs="['Home','Offer']" />
<x-spacer></x-spacer>
...
<div class="container w-full mx-auto flex gap-12 items-start relative">
    <div class="w-8/12">
        ...
        <div class="w-full">
            <div class="rounded-t-xl px-6 py-3 pb-5 w-full bg-emerald-300 text-base">
                <span class="font-semibold">{{ __('Direct line with') }} {{  $flight->airline->name }}</span>
            </div>
            <div class="-mt-3">
                <div
                    ...
                    <span class="font-black text-lg">{{__('going')}}</span>
                ...
                <div
                    ...
                    <span class="font-black text-lg">{{__('Return')}}</span>
                    <span class="dots flex-1 border-b-2 mx-3 border-black-500 h-full"></span>
                    <span class="flex items-center">
                        <img src="{{ Vite::asset('resources/images/plane.webp') }}" alt="{{__('Plane')}}"
        ...
        <x-spacer></x-spacer>
        <div class="w-full">
            <span class="block text-2xl font-bold w-full block mb-6">{{__('Features of this offer')}}</span>
                ...
                @foreach ([1, 2, 3, 4, 5, 6] as $item)
                    <div class="flex items-start justify-start w-full gap-3">
                        ...
                        <div class="flex flex-col justify-start items-start w-8/12">
                            <span class="text-lg font-semibold">{{__('Accommodation')}}</span>
        ...
        <x-spacer></x-spacer>
        <div class="w-full">
            <span class="block text-2xl font-bold w-full block mb-6">{{__('Offer Description')}}</span>
        ...
        <div class="w-full" id="packages">
            <span class="block text-2xl font-bold w-full block mb-6">{{__('Available bookings')}}</span>
    ...
    <div class="w-4/12 h-auto sticky top-10">
        <div class="border border-black-200 bg-white rounded-3xl p-6 mb-6 w-full">
            <span class="block opacity-45 text-base font-bold line-through line-clamp-2 lining-nums">{{ $flight->old_price }}
                {{__('dh')}}</span>
            <span class="text-3xl block font-black lining-nums">{{ $flight->price }} {{__('dh')}} {{__('For a person')}}</span>
            <span
            ...
            <div class="mb-6">
                <span class="text-base font-base block mb-3 flex items-center">
                    ...
                    <span>
                        {{__('Book from any area in Morocco')}}
                    </span>
                </span>
                <span class="text-base font-base block mb-3 flex items-center">
                    <img src="{{Vite::asset('resources/images/checked.webp')}}" alt="Offer"
                        class="w-6 h-6 inline-block text-emerald-550 me-2">
                    <span>
                        {{__('Customer service center available 24/7')}}
                    </span>
                </span>
                <span class="text-base font-base block mb-3 flex items-center">
                    <img src="{{Vite::asset('resources/images/checked.webp')}}" alt="Offer"
                        class="w-6 h-6 inline-block text-emerald-550 me-2">
                    <span>
                        {{__('High quality services at competitive prices')}}
                    </span>
                </span>
            </div>
            <div class="flex flex-col">
                <a href="#packages"
                    class="btn btn-lg text-xl rounded-xl hover:bg-yellow-600 hover:shadow-lg font-black from-yellow-600  bg-gradient-to-l to-yellow-500 mb-3">{{__('Choose the package')}}</a>
                <button
                ...
                <a href="{{route('faqs')}}" class="btn btn-lg text-xl rounded-xl border border-black-200 hover:shadow-lg font-black mb-3">{{__('You have a question?')}}</a>
            </div>
        </div>
...
<div class="container m-auto flex justify-between items-start">
    <div class="w-4/12 flex items-start justify-center">
        <x-text-offer-card icon="{{ Vite::asset('resources/images/booking.webp') }}" :description="__('Effortless Booking Anywhere in Morocco')" :title="__('Book Umrah from anywhere in Morocco')"></x-text-offer-card>
    </div>
    <div class="w-4/12 flex items-start justify-center">
        <x-text-offer-card icon="{{ Vite::asset('resources/images/customer-offer.webp') }}" :description="__('Exclusive Packages and Seasonal Offers at Competitive Prices')"
            :title="__('Special packages and offers throughout the year')"></x-text-offer-card>
    </div>
    <div class="w-4/12 flex items-start justify-center">
        <x-text-offer-card icon="{{ Vite::asset('resources/images/customer-service.webp') }}" :description="__('Our team is ready to answer all your inquiries and provide assistance at any time to ensure the comfort and tranquility of the pilgrims.')" :title="__('24/7 support service')"></x-text-offer-card>
    </div>
</div>
<x-spacer></x-spacer>
<x-footer></x-footer>
