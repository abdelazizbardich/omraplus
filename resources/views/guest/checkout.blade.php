<x-guest-layout>
        ...
        <div class="container w-full mx-auto flex gap-12 items-start relative">
            <div class="w-8/12">
                <div class="rounded-lg shadow-md bg-white mb-6">
                    ...
                    <div class="flex items-center justify-between px-3 mb-3 pb-3">
                        <span class="font-black">{{ __('Pilgrims count') }}</span>
                    ...
                    <div class="flex flex-col items-center justify-between px-3 mb-3 pb-3">
                        <div class="collapse rounded-none flex items-center">
                            ...
                            <label
                                ...
                                for="PAY_LATER">
                                <span class="flex items-center">{{ __('Book Now & Pay Later') }}</span>
                            </label>
                        </div>
                        <div class="collapse rounded-none">
                            ...
                            <label class="border border-black-200 peer-checked:from-yellow-500  peer-checked:bg-gradient-to-l peer-checked:to-yellow-400 rounded-lg p-3 w-full mb-3 cursor-pointer hover:shadow-md h-16 collapse-title flex items-center justify-between w-full cursor-pointer"
                                for="BANK_TRANSFER">
                            ...
                            <div class="collapse-content text-sm">
                                <div class="flex gap-3 mb-3">
                                    <div class="w-1/2">
                                        ...
                                        <span class="block" dir="rtl"><span
                                                class="font-bold">{{ __('Bank name') }}:</span>Wafa
                                            Bank</span>
                                        <span class="block" dir="rtl"><span
                                                class="font-bold">{{ __('Account Number') }}:</span>
                                            123456789</span>
                                    </div>
                                    <div class="w-1/2">
                                ...
                                <span
                                    class="block">{{ __('Please send a photo of the receipt to the following number to confirm the payment.') }}</span>
                                <span class="block w-fit font-semibold text-lg"
                                    dir="ltr">{{ __('+212 6 12 34 56 78') }}</span>
                            </div>
                        </div>
                        <div class="collapse rounded-none">
                                ...
                                <div class="w-1/2 min-w-72">
                                    <div class="mb-3">
                                        <fieldset class="fieldset">
                                            <legend class="fieldset-legend">{{ __('Card number') }}</legend>
                                            <input name="card_number" type="text" class="input w-full" type="text" inputmode="numeric" oninput="this.value = this.value.replace(/[^0-9\s]/g, '')" maxlength="19" placeholder="xxxx xxxx xxxx xxxx"  />
                                        </fieldset>
                                    </div>
                                    <div class="flex gap-3 w-full">
                                        <fieldset class="fieldset w-full">
                                            <legend class="fieldset-legend">{{ __('Expiry date') }}</legend>
                                            <input name="expiry_date" type="month" class="input w-full" placeholder="" />
                                        </fieldset>
                                        <fieldset class="fieldset w-full">
                                            <legend class="fieldset-legend">{{ __('CVC code') }}</legend>
                                            <input name="cvc_code" type="text" oninput="this.value = this.value.replace(/[^0-9\s]/g, '')"  maxlength="3" minlength="3" class="input w-full" inputmode="numeric" placeholder="xxx" />
                                        </fieldset>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="collapse rounded-none">
                            ...
                            <div class="collapse-content text-sm">
                                <span class="block" dir="rtl"><span class="font-bold">{{ __('Account Number') }}:</span> 123456789</span>
                                <span class="block">{{ __('Please send a photo of the receipt to the following number to confirm the payment.') }}</span>
                                <span class="block w-fit font-semibold text-lg" dir="ltr">{{ __('+212 6 12 34 56 78') }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="w-4/12 h-auto sticky top-10">
                <div class="border border-black-200 bg-white rounded-3xl p-6 mb-6 w-full">
                    ...
                    <div class="flex">
                        <span class="w-6/12">{{__('Adult')}}: 1 x {{ __('dh') }} {{ $price->price }}</span>
                        <span class="w-6/12 font-bold" id="price">{{ $price->price }} {{ __('dh') }}</span>
                    </div>
                    <hr class="my-3 opacity-30">
                    <div class="flex mb-4">
                        <span class="w-6/12">{{__('Total')}}:</span>
                        <span class="w-6/12 font-bold" id="total-price">{{ $price->price }} {{ __('dh') }}</span>
                    </div>
                    <button
                        class="w-full btn btn-lg text-xl rounded-xl hover:bg-yellow-600 hover:shadow-lg font-black from-yellow-600  bg-gradient-to-l to-yellow-500">{{ __('Book now') }}</button>
                </div>
                <x-spacer></x-spacer>
            </div>
        </div>
    ...
    <script>
        ...
        function updatePrice() {
            ...
            price.innerText = parseFloat(total).toFixed(2) + ' {{ __('dh') }}';
            totalPrice.innerText = parseFloat(total).toFixed(2) + ' {{ __('dh') }}';
        }
