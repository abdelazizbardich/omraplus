...
@section('content')
    ...
    <div class="container m-auto flex justify-between items-start">
        <div class="w-4/12 flex items-start justify-center">
            <x-atoms.text-offer-card :index="'0'" icon="{{ Vite::asset('resources/images/booking.webp') }}" :description="__('Effortless Booking Anywhere in Morocco')" :title="__('Book Umrah from anywhere in Morocco')"></x-atoms.text-offer-card>
        </div>
        <div class="w-4/12 flex items-start justify-center">
            <x-atoms.text-offer-card :index="'1'" icon="{{ Vite::asset('resources/images/customer-offer.webp') }}" :description="__('Exclusive Packages and Seasonal Offers at Competitive Prices')" :title="__('Special packages and offers throughout the year')"></x-atoms.text-offer-card>
        </div>
        <div class="w-4/12 flex items-start justify-center">
            <x-atoms.text-offer-card :index="'2'" icon="{{ Vite::asset('resources/images/customer-service.webp') }}" :description="__('Our team is ready to answer all your inquiries and provide assistance at any time to ensure the comfort and tranquility of the pilgrims.')" :title="__('24/7 support service')"></x-atoms.text-offer-card>
        </div>
    </div>
    <x-atoms.spacer></x-atoms.spacer>
    <div class="container m-auto flex justify-between items-start">
