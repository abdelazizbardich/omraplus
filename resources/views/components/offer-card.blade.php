    ...
    <div class="flex flex-col h-full border bg-white border-black-200 hover:shadow-md duration-300 rounded-3xl w-full h-full mt-3 overflow-hidden relative">
        <div class="w-full relative mb-8 bg-black-300">
            <a href="{{route('flight', $data->id)}}">
                <img src="{{ @$data->photos()->where('is_main',1)->first()->url }}" class="w-full aspect-video object-cover" alt="umrah">
            </a>
        ...
        <div class="p-3 flex flex-col h-full">
            ...
            <div class="flex flex-col mb-3">
                <span class="block opacity-45 text-xs font-bold line-through line-clamp-2 lining-nums">{{ money(@$data->getLowestPrice()->old_price) }} {{__('dh')}}</span>
                <span class="text-xs block font-black lining-nums"><span class="text-3xl">{{ money(@$data->getLowestPrice()->price) }}</span> {{__('dh')}} {{__('For a person')}}</span>
