                    ...
                    <div class="flex justify-start flex-wrap items-center gap-3">
                        ...
                        @foreach ($booking->prices as $index=>$price)
                            <div class="h-auto w-fit">
                                <input @if($index==0) checked @endif hidden required class="peer hidden room-type" data-price="{{$price->price}}" data-old-price="{{$price->old_price}}" value="{{$price->room->id}}" type="radio" name="room_type" id="room-{{$price->room->id}}">
