            ...
            <div class="relative z-10 flex items-stretch justify-between w-full text-lg px-6 py-4 bg-white border border-black-200 rounded-xl shadow-md">
                ...
                <div class="flex flex-col justify-between items-start border-s border-black-200 ms-6 ps-6">
                    <div class="w-full mb-6">
                        <span class="block opacity-45 text-base font-bold line-through line-clamp-2 lining-nums">{{count($booking->prices)?$booking->prices[0]->old_price:0}} {{__('dh')}}</span>
                        <span class="text-xl block font-black lining-nums"><span class="text-3xl">{{count($booking->prices)?$booking->prices[0]->price:0}}</span> {{__('dh')}} {{__('For a person')}}</span>
